...
const AddSkill = () => {
  ...
  const addSkill = () => {
    const existskill = mySkill.indexOf(skillname[`skillname`]) ? setMySkill([skillname[`skillname`], ...mySkill]) : alert('ja existe')
  
    localStorage.setItem('skills', JSON.stringify([skillname[`skillname`], ...mySkill]))
  ...
  const createSkill = () => {
      api.post(`resume/skills`, skillname , permissions)
      .then(success => {
        const {data } = success
      ...
      <Card>
        <div className="row">
          <div className="column">
            ...
            {mySkill.map(skill => {
              return(<div className="label" key={skill}>{skill} <MdClose onClick={() => removeSkill(skill)}/></div>)
            })} 
          </div>
        </div>
        <div className="column">
          <div>
              <label>Adicionar competencia:</label>
                ...
                <input list="skills" name="skillname" id="skill" type="search" onChange={addNewSkill} />
                    <datalist id="skills">
                        {skills.map(skill => <option key={skill.id} value={skill.skillname} /> )}
                    </datalist>
